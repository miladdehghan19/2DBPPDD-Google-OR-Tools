...
static void Solving_Discrete_Bins()
{
    ...
    for (int j = 0; j < m; j++)
    {
        ...
        for (int i = 0; i < ntotal; i++)
        {
            ...
            clap.AddRectangle(width_iv[i],height_iv[i]); //Why NewOptionalIntervalVar????            
...
static void Solving_Best_Continous_Bin()
{
    ...
    BoolVar[] flag=new BoolVar[ntotal];
    for (int i = 0; i < ntotal; i++)
    {
        flag[i]=model.NewBoolVar($"flag[{i}]");  
    }
    ...
    //Solve
    CpSolver solver=new CpSolver();
    solver.StringParameters="log_search_progress : true, num_search_workers:8";
    CpSolverStatus status=solver.Solve(model);
    System.Console.WriteLine(status);
