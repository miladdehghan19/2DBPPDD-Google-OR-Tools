...
static void Solving_Better_Continous_Bin()
{
    ...
    for (int i = 0; i < n; i++)
    {
        //left[i] is the left coordinate of item i and is equal to left_fix[i] or left_rot[i]        
        left[i]=model.NewIntVar(0,(m*sizeb[0])-Math.Min(sizei[i,0],sizei[i,1]),$"left[{i}]");
        //right[i] is the right coordinate of item i
        right[i]=model.NewIntVar(Math.Min(sizei[i,0],sizei[i,1]),m*sizeb[0],$"right[{i}]");
        
        //left_fix[i] is the left coordinate of item i if it be fixed
        left_fix[i]=model.NewIntVarFromDomain(Domain.FromValues(xdomain_left_fix[i]),$"left_fix[{i}]");
        //left_rot[i] is the left coordinate of item i if it be rotated 
        ...
        //sizei[i,0]=width of item i---sizei[i,1]=height of item i
        model.Add(left[i]+sizei[i,0]==right[i]).OnlyEnforceIf(flag[i].Not());
        model.Add(left[i]+sizei[i,1]==right[i]).OnlyEnforceIf(flag[i]);

        //bottom[i] is the bottom coordinate of item i---top[i] is the top coordinate of item i
        bottom[i]=model.NewIntVar(0,sizeb[1]-Math.Min(sizei[i,1],sizei[i,0]),$"bottom{i}");
        top[i]=model.NewIntVar(Math.Min(sizei[i,1],sizei[i,0]),sizeb[1],$"top{i}");
        model.Add(bottom[i]+sizei[i,1]==top[i]).OnlyEnforceIf(flag[i].Not());
        model.Add(bottom[i]+sizei[i,0]==top[i]).OnlyEnforceIf(flag[i]);
    ...
    //Objective Function
    IntVar lmax=model.NewIntVar(ptimeb-di.Max()-1,(n*ptimeb)-di.Min()+1,"lmax");
    //l[i] is the lateness of item i
    IntVar[] l=new IntVar[n];
    //b[i] is the number of bin that item i assigned
...
//-------------------------------------------------------------------------------------------------------
/*
//gets correct OF but wrong left,right,bottom and top of items
static void Solving_Better_Discrete_Bins()
{
    //Parameters
    int[] sizeb={10,10}; //W-H
    int[,] sizei={{9 , 5},
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            for (int k = 0; k < 2; k++)
            {
                f[i,j,k]=model.NewBoolVar($"f[{i},{j},{k}]");
            }
    ...
    for (int j = 0; j < m; j++)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            left[i]=model.NewIntVar(0,sizeb[0]-Math.Min(sizei[i,0],sizei[i,1]),$"left{i}");
            right[i]=model.NewIntVar(Math.Min(sizei[i,0],sizei[i,1]),sizeb[0],$"right{i}");
            model.Add(left[i]+sizei[i,0]==right[i]).OnlyEnforceIf(f[i,j,0]);
            model.Add(left[i]+sizei[i,1]==right[i]).OnlyEnforceIf(f[i,j,1]);

            bottom[i]=model.NewIntVar(0,sizeb[1]-Math.Min(sizei[i,1],sizei[i,0]),$"bottom{i}");
            top[i]=model.NewIntVar(Math.Min(sizei[i,1],sizei[i,0]),sizeb[1],$"top{i}");
            model.Add(bottom[i]+sizei[i,1]==top[i]).OnlyEnforceIf(f[i,j,0]);
            model.Add(bottom[i]+sizei[i,0]==top[i]).OnlyEnforceIf(f[i,j,1]);

            //width_iv[i]=model.NewIntervalVar(left[i],sizeitotal[i,0],right[i],$"width_iv[{i}]");
            //height_iv[i]=model.NewIntervalVar(bottom[i],sizeitotal[i,1],top[i],$"height_iv[{i}]");
            
            width_iv[i]=model.NewOptionalIntervalVar(left[i],sizei[i,0],right[i],f[i,j,0],$"width_iv[{i}]");
            height_iv[i]=model.NewOptionalIntervalVar(bottom[i],sizei[i,1],top[i],f[i,j,0],$"height_iv[{i}]");

            width_iv_II[i]=model.NewOptionalIntervalVar(left[i],sizei[i,1],right[i],f[i,j,1],$"width_iv[{i}]");
            height_iv_II[i]=model.NewOptionalIntervalVar(bottom[i],sizei[i,0],top[i],f[i,j,1],$"height_iv[{i}]");
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < m; j++)
        {
            model.Add(l[i]==((j+1)*ptimeb)-di[i]).OnlyEnforceIf(f[i,j,0]);
            model.Add(l[i]==((j+1)*ptimeb)-di[i]).OnlyEnforceIf(f[i,j,1]);
        }
    }
    ...
    if (status==CpSolverStatus.Optimal || status==CpSolverStatus.Feasible)
    {
        for (int j = 0; j < m; j++)
        {
            for (int i = 0; i < n; i++)
            {
                for (int k = 0; k < 2; k++)
                {
                  if (solver.Value(f[i,j,k])==1)
                  {
                    System.Console.WriteLine($"Bin[{j}]");
                    System.Console.WriteLine($"f[{i},{j},{k}]={solver.Value(f[i,j,k])}");
                    System.Console.WriteLine($"left[{i}]={solver.Value(left[i])}---right[{i}]={solver.Value(right[i])}");
                    System.Console.WriteLine($"bottom[{i}]={solver.Value(bottom[i])}---top[{i}]={solver.Value(top[i])}");
                    System.Console.WriteLine("");
                  }  
